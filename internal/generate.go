...
func Generate(m *StructMapping, goPackage string, goFile string, w io.Writer) (outputFilename string, err error) {
	...
	f.ImportAlias("github.com/networkteam/construct/v2/json", "cjson")
	f.ImportAlias("github.com/networkteam/construct/v2", "construct")
...
func generateDefaultSelectJsonObject(f *File, m *StructMapping) {
	...
	code := Qual("github.com/networkteam/construct/v2/json", "JsonBuildObject").Call()
	for _, fm := range m.FieldMappings {
		if fm.ReadColDef != nil {
			...
			code.Op(".").Line().Id("Set").Call(Lit(fm.Name), Qual("github.com/networkteam/construct/v2/json", "Exp").Call(setExpValue))
		}
	}
...
func generateScanJsonRow(f *File, m *StructMapping) {
	f.Func().Id(firstToLower(m.TargetName)+"ScanJsonRow").Params(
		Id("row").Qual("github.com/networkteam/construct/v2", "RowScanner"),
	).Params(
		...
		If(
			...
			If(Id("err").Op("==").Qual("database/sql", "ErrNoRows")).Block(
				Return(Id("result"), Qual("github.com/networkteam/construct/v2", "ErrNotFound")),
			),
